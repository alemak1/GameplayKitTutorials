...
class GKSpikemanRight: GKEnemy{
    ...
    init(scalingFactor: CGFloat, position: CGPoint?, movementLowerBound: CGFloat?, movementUpperBound: CGFloat?) {
        ...
        let randomRightEdgePoint = RandomGenerator.getRandomEdgePointFor(screenEdge: .Right)
        let spikeManPos = position ?? randomRightEdgePoint
        ...
        let moveAction1: SKAction = (movementUpperBound == nil) ?
            ...
            SKAction.move(to: CGPoint(x: ScreenPoints.TopRightCorner.x , y: movementUpperBound!), duration: 2.00)
        
        let moveAction2: SKAction = (movementLowerBound == nil) ?
            SKAction.move(to: ScreenPoints.BottomRightCorner, duration: 2.00) :
            SKAction.move(to: CGPoint(x: ScreenPoints.BottomRightCorner.x , y: movementLowerBound!), duration: 2.00)
